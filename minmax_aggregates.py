...
class MinMaxAggregator:
    ...
    def _replace_results_in_minimize(self, stm, minimizes):
        ...
        for aggtype, translation, idx in temp_minmax_preds:
            ...
            old_vars = set(map(lambda x: x.name, collect_ast(oldmax, "Variable"))) - {
                varname
            }
    ...
    def _replace_results_in_sum(self, stm):
        ...
        for b in stm.body:
            if (
                ...
                and b.atom.function
                in (AggregateFunction.Sum, AggregateFunction.SumPlus)
            ):
                ...
                for elem in atom.elements:
                    ...
                    l = set(
                        chain.from_iterable(
                            predicates(b, {Sign.NoSign, Sign.DoubleNegation})
                    ...
                    for aggtype, translation, idx in temp_minmax_preds:
                        ...
                        term_vars = chain.from_iterable(
                            map(_characteristic_variables, term_tuple[1:])
                        )
                        term_vars = {x.name for x in term_vars}
                        if not old_vars <= term_vars:
                            elements.append(elem)
                        ...
                        newargs = [
                            next_ if i == idx else x for i, x in enumerate(newargs)
                        ]
                        chainpred = Literal(
                            loc,
                            Sign.NoSign,
                        ...
                        nextpred = Literal(
                            ...
                            SymbolicAtom(
                                Function(
                                    loc,
                                    self.domain_predicates.next_predicate(dompred, 0)[
                                        0
                                    ],
                                    [PREV, NEXT],
                                    False,
                        ...
                        elements.append(
                            BodyAggregateElement(
                                [weight] + terms, [chainpred, nextpred] + body
                            )
                        )
                        # __NEXT, __chain__max_0_0_x(#sup,__NEXT) : __chain__max_0_0_x(P,__NEXT), __min_0__dom___max_0_0_x(__NEXT)
                        infsup = Infimum
                        ...
                        if aggtype == AggregateFunction.Max:
                            minmaxpred = self.domain_predicates.min_predicate(
                                dompred, 0
                            )[0]
                        else:
                            minmaxpred = self.domain_predicates.max_predicate(
                                dompred, 0
                            )[0]
                        minmaxlit = Literal(
                            loc,
                        ...
                        elements.append(
                            BodyAggregateElement(
                                [weight] + terms, [chainpred, minmaxlit] + body
                            )
                        )
                        # #inf and #sup are ignored by minimized and therefore not included
                        # (also would require more complex variable bindings)
                ...
                agg = BodyAggregate(
                    loc, atom.left_guard, atom.function, elements, atom.right_guard
                )
